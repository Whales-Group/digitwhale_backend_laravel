...
class PackageService
{
    ...
    private function performSecurityChecks(?string $packageType = null): Account
    {
        ...
        if (!$account->enabled) {
            throw new Exception(
                "[Account Disabled] - Cannot perform package operations. Contact customer support",
                403
            );
        }
        ...
        if ($account->blacklisted) {
            throw new Exception(
                "[Account Blacklisted] - Cannot perform package operations. MESSAGE: " .
                    $account->blacklist_text,
                403
            );
        }
        ...
        if ($account->pnd) {
            throw new Exception(
                "Account cannot perform operations at the moment due to PND status. Contact customer support",
                403
            );
        }
        ...
        if ($packageType) {
            ...
            if ($packagePrice > 0 && $account->balance < $packagePrice) {
                throw new Exception(
                    "Insufficient funds. Please top up your account balance.",
                    400
                );
            }
    ...
    public function subscribe(string $packageType): JsonResponse
    {
        try {
            ...
            $lock = Cache::lock(
                "subscribe_{$user->id}",
                Cred::LOCK_TIME_IN_SECONDS->value
            ); // Lock for Cred::LOCK_TIME_IN_SECONDS->value seconds

            if (!$lock->get()) {
                throw new Exception(
                    "Another subscription operation is in progress. Please try again.",
                    429
                );
            }
            ...
            try {
                ...
                return DB::transaction(function () use (
                    ...
                    if ($package->price !== "0/month") {
                        $packagePrice = (float) str_replace(
                            "/month",
                            "",
                            $package->price
                        );
                        $transactionData = [
                            ...
                            "to_user_name" => "WhaleGPT Upgrade",
                            "to_user_email" => "system@whalegpt.com",
                            "to_bank_name" => "WhaleGPT Upgrade",
                            "to_bank_code" => "00",
    ...
    public function unsubscribe(): JsonResponse
    {
        try {
            $user = Auth::user();
            $lock = Cache::lock(
                "unsubscribe_{$user->id}",
                Cred::LOCK_TIME_IN_SECONDS->value
            );

            if (!$lock->get()) {
                throw new Exception(
                    "Another unsubscribe operation is in progress. Please try again.",
                    429
                );
            }
    ...
    public function upgrade(string $newPackageType): JsonResponse
    {
        try {
            ...
            if (!$lock->get()) {
                throw new Exception(
                    "Another upgrade operation is in progress. Please try again.",
                    429
                );
            }
            ...
            try {
                ...
                return DB::transaction(function () use (
                    ...
                    $currentPrice = (float) str_replace(
                        "/month",
                        "",
                        $currentPackage->price
                    );
                    $newPrice = (float) str_replace(
                        "/month",
                        "",
                        $newPackage->price
                    );
                    ...
                    if ($newPrice <= $currentPrice) {
                        throw new Exception(
                            "New package must have a higher price for upgrade",
                            400
                        );
                    }
                    ...
                    if ($additionalCost > $account->balance) {
                        throw new Exception(
                            "Insufficient funds for upgrade. Please top up your account.",
                            400
                        );
                    }
    ...
    public function downgrade(string $newPackageType): JsonResponse
    {
        try {
            ...
            if (!$lock->get()) {
                throw new Exception(
                    "Another downgrade operation is in progress. Please try again.",
                    429
                );
            }
            ...
            try {
                ...
                return DB::transaction(function () use ($user, $newPackage) {
                    ...
                    if ($newPrice >= $currentPrice) {
                        throw new Exception(
                            "New package must have a lower price for downgrade",
                            400
                        );
                    }
